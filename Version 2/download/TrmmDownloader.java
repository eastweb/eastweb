...
public class TrmmDownloader extends Downloader {
    ...
    public TrmmDownloader(TrmmProduct product, DataDate date, File outFile)
            throws IOException {
        mProduct = product;
        mDate = date;
        mOutFile = outFile;
     ...
     * @throws IOException
     */
    public static final List<DataDate> listDates(TrmmProduct product,
            DataDate startDate) throws ConfigReadException, IOException {
        ...
        if (mode == Mode.FTP) {
            ...
            try {
                ...
                System.out
                .println("Can't connect to Trmm data website, please check your URL.");
                return null;
            }
            ...
            try {
                ...
                for (FTPFile yearFile : ftp.listFiles()) {
                    ...
                    for (FTPFile file : ftp.listFiles()) {
                        if (file.isFile()
                                && Settings.getTrmmPattern(product)
                                .matcher(file.getName()).matches()) {
                            // Assume following format: {product
                            // name}.%y4.%m2.%d2.7.bin
    ...
    public final void download() throws IOException {
        ...
        if (mode == Mode.FTP) {
            ...
            try {
                final String yearDirectory =
                        String.format("%s/%04d", Settings.getRootDir(mProduct),
                                mDate.getYear());
                if (!ftp.changeWorkingDirectory(yearDirectory)) {
                    throw new IOException("Couldn't navigate to directory: "
                            + yearDirectory);
                ...
                DownloadUtils.download(ftp,
                        Settings.getFilename(mProduct, mDate), mOutFile);
