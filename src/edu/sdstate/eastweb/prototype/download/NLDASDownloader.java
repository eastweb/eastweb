...
public final class NLDASDownloader extends Downloader {
    ...
    public static final List<DataDate> listDates(DataDate startDate)
            throws ConfigReadException, IOException {
        Mode mode = Settings.getMode(DataType.NLDAS);
        if (mode == Mode.FTP) {
            // TODO: Please check the patterns to make sure that they are
            // compatible.
            ...
            try {
                ...
                System.out
                .println("Can't connect to NLDAS data website, please check your URL.");
                return null;
            }
            ...
            try {
                ...
                for (FTPFile yearFile : ftp.listFiles()) {
                    ...
                    /*
                     * NLDS file have different file structure. list days need
                     * be rewrite. //TODO: Create your pattern here Pattern
                     * mpattern=Pattern.compile(
                     * "NLDAS_FORA0125_H\\.A(\\d{4})(\\d{2})(\\d{2})\\.(\\d{4})\\.002\\.grb"
                     * ); for (FTPFile file : ftp.listFiles()) { if
                     * (file.isFile() &&
                     * mpattern.matcher(file.getName()).matches()) { // TODO:
                     * Assume following format is {product
                     * name}.%y4.%m2.%d2.7.bin, please change it as needed
                     * String[] strings = file.getName().split("[.]"); final int
                     * month = Integer.parseInt(strings[2]); final int day =
                     * Integer.parseInt(strings[3]); final DataDate dataDate =
                     * new DataDate(day, month, year); if
                     * (dataDate.compareTo(startDate) >= 0) {
                     * list.add(dataDate); } } }
                     */
            ...
            URLConnection conn =
                    (URLConnection) ConnectionContext.getConnection(mode,
                            DataType.NLDAS);
            // TODO: Please check the pattern to make sure that it is
            // compatible.
            final Pattern re =
            ...
            try {
                ...
                throw new IOException(
                        "Failed to parse the NLDAS download page", e);
            }
            ...
            for (int i = 0; i < dirlist.getLength(); ++i) {
                ...
                if (matcher.matches()) {
                    // TODO: Please change this part if you have different
                    // pattern string
    ...
    DownloadFailedException {
        ...
        if (mode == Mode.FTP) {
            ...
            String url_str =
                    Config.getInstance().getNLDASUrl()
                    + String.format("%04d.%02d.%02d", mDate.getYear(),
                            mDate.getMonth(), mDate.getDay());
